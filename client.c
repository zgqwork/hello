...
int main( int argc, char ** argv )
{
	...
	int fd[2], nbytes;
	pid_t pid;
	char string[WRITELINE] = "hello my pipe 2014!";
	...
	if(pid==0)
	{
		...
		while(1)
		{
			...
			getsline(line);
			printf("You enter:%s\n",line);
			result = write(*write_fd, line, strlen(line));
		}
		return 0;
	}
	...
	else
	{
		...
		for(i=0;i<cCount;i++)
		{
			if((connfd[i] = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
				printf("Socket Error...\n" , errno );
				exit( EXIT_FAILURE );
			}
		...
		while(1)
		{
			...
			for(i=0;i<cCount;i++) {
				sprintf(buf, "%s--fd=%d", readbuffer, connfd[i]);
